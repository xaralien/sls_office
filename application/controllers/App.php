...
class App extends CI_Controller
{
	...
	private function get_inbox_counts($nip)
	{
		// Get memo count
		$sql = "SELECT COUNT(Id) as count FROM memo WHERE (nip_kpd LIKE '%$nip%' OR nip_cc LIKE '%$nip%') AND (`read` NOT LIKE '%$nip%')";
		$query = $this->db->query($sql);
		$memo_count = $query->row()->count;

		// Get task count
		$sql = "SELECT COUNT(id) as count FROM task WHERE (`member` LIKE '%$nip%' or `pic` LIKE '%$nip%') and activity='1'";
		$query = $this->db->query($sql);
		$task_count = $query->row()->count;

		return ['count_inbox' => $memo_count, 'count_inbox2' => $task_count];
	}

	private function check_user_level()
	{
		return strpos($this->session->userdata('level'), '401') !== false;
	}

	public function create_memo()
	{
		if ($this->check_user_level()) {
			$data['sendto'] = $this->m_app->sendto($this->session->userdata('level_jabatan'), $this->session->userdata('bagian'));

			$nip = $this->session->userdata('nip');
			$counts = $this->get_inbox_counts($nip);
			$data = array_merge($data, $counts);

			// $this->load->view('create_memo', $data);
			$data['title'] = "Create memo";
			$data['pages'] = "pages/memo/v_create_memo";
	...
	public function memo_pdf()
	{
		...
		if (strpos($a, '302') !== false) {
			...
			$this->load->view('pages/memo/memo_pdf', $data);
		}
	}
