...
function update_task()
{
    ...
    public function save_detail_task()
    {
        ...
        for ($i=1; $i <=$total_row ; $i++) { 
			// echo var_dump($_FILES['att'.$i]['name']);
			// $ext = explode('.', $_FILES['att'.$i]['name']);
			// $end_ext = end($ext);
			$target_file = './upload/card_task/';
			$target_file2 = './upload/task_comment/';
			// move_uploaded_file($_FILES["foto_asset"]["tmp_name"], $target_file);

			// $temp = explode(".", $_FILES['att'.$i]["name"]);
			// echo var_dump($_FILES['att'.$i]['name'][0]);
			// if (count($_FILES['att'.$i]['name'][0]) == 0) {
			// 	echo 1;
			// }else{
			// 	echo 2;
			// }
